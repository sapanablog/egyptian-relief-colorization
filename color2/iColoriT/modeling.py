...
# # #         self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
# # #
# # #     def forward_features(self, x, hint_mask):
# # #         # x: [B, 3, H, W] (L + ab hints)
# # #         # hint_mask: [B, 1, H, W]
# # #         B, _, H, W = x.shape
# # #         assert hint_mask.shape == (B, 1, H, W), f"Expected hint_mask of shape [B,1,{H},{W}], got {hint_mask.shape}"
# # #
# # #         full_mask = hint_mask.bool()
# # #
# # #         if self.avg_hint:
# # #             # Optional avg_hint logic – you likely don't need it
# # #             raise NotImplementedError("avg_hint mode not supported for full-res binary masks")
# # #         else:
# # #             # Zero out ab channels where mask == 0
# # #             x[:, 1, :, :].masked_fill_(~full_mask.squeeze(1), 0)
# # #             x[:, 2, :, :].masked_fill_(~full_mask.squeeze(1), 0)
# # #
# # #         if self.in_chans == 4:
# # #             x = torch.cat((x, full_mask), dim=1)  # add mask as 4th channel
# # #
# # #         x = self.patch_embed(x)
...
# # #         return x
# # #
# # #     def forward(self, x, hint_ab, hint_mask):
# # #         x = torch.cat([x, hint_ab], dim=1)  # Combine L + ab
# # #         x = self.forward_features(x, hint_mask)
# # #         x = self.head(x)
# # #         x = self.tanh(x)
# # #         return x
# # # '''
...
# # #     if pretrained:
# # #         init_ckpt = kwargs.get("init_ckpt", None)  # Avoid KeyError
# # #         if init_ckpt:
# # #             checkpoint = torch.load(init_ckpt, map_location="cpu")
# # #             model.load_state_dict(checkpoint["model"])
# # #         else:
# # #             print("Warning: No init_ckpt provided, loading default pre-trained weights.")
# # #
# # #     return model
# # #
# # #
...
# #
# #
# # ##############this below is done for patch for horizontal ,square
# #
# # # --------------------------------------------------------
# # # Based on BEiT, timm, DINO and DeiT code bases
# # # https://github.com/microsoft/unilm/tree/master/beit
...
# #
# # class LocalAttentionHead(nn.Module):
# #     def __init__(self, dim, out_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
# #             proj_drop=0., attn_head_dim=None, use_rpb=False, window_size=14):
# #         super().__init__()
# #         self.num_heads = num_heads
# #         head_dim = dim // num_heads
...
# #
# #     def forward_features(self, x, mask):
# #         # --- PATCH-MASK to PIXEL-MASK logic ----
# #         B, _, H, W = x.shape
# #         assert mask.dim() == 2, f'Expected patch mask of shape [B, n_patches], got {mask.shape}'
# #         n_patches = mask.shape[1]
# #         side = int(math.sqrt(n_patches))
# #         assert side * side == n_patches, "n_patches must be a square number!"
# #         mask_2d = mask.view(B, 1, side, side)
# #         full_mask = F.interpolate(mask_2d, size=(H, W), mode="nearest")
...
# #         if self.in_chans == 4:
# #             pass
# #             #x = torch.cat((x, full_mask.float()), dim=1)
# #         x = self.patch_embed(x)
# #         x = x + self.pos_embed.type_as(x).to(x.device).clone().detach()
# #         for blk in self.blocks:
# #             x = blk(x)
...
# #
# #     def forward(self, x, mask):
# #         x = self.forward_features(x, mask)  # [B, n_patches, embed_dim]
# #         x = self.head(x)  # [B, n_patches, patch_area * 2]
# #         B, N, D = x.shape
# #         patch_area = (self.patch_size if isinstance(self.patch_size, int) else self.patch_size[0]) ** 2
# #         x = x.view(B, N, patch_area, 2)  # [B, n_patches, patch_area, 2]
# #         x = self.tanh(x)
# #         return x
# #
# #
# # # --- All the @register_model functions (no changes) ---
# #
# # @register_model
# # def icolorit_tiny_4ch_patch8_224(pretrained=False, **kwargs):
# #     model = IColoriT(
...
# #     model.default_cfg = _cfg()
# #     if pretrained:
# #         init_ckpt = kwargs.get("init_ckpt", None)
# #         if init_ckpt:
# #             checkpoint = torch.load(init_ckpt, map_location="cpu")
# #             model.load_state_dict(checkpoint["model"])
# #         else:
...
# #             print("Warning: No init_ckpt provided, loading default pre-trained weights.")
# #     return model
# ############################this is modified by me on 24th june for L,ab,hint_mask,G_ab
#
#
# #######################
#
...
#             print("WARNING: Using avg_hint mode with full-res binary masks.")
#
#         else:
#             x[:, 1, :, :].masked_fill_(~full_mask.squeeze(1), 0)
#             x[:, 2, :, :].masked_fill_(~full_mask.squeeze(1), 0)
...
##the above code is for ab, l, hint mask for 4 parameter

##############################################################################################################################
##################################################################
##########################################the below code  is for prev icolorit model but uses patches but colored with random hints same like icolorit model
...
class IColoriT(nn.Module):
    ...
    def forward_features(self, x, mask):
        ...
        ###i have modified for patches with hints 20
        #full_mask = F.interpolate(mask_2d.float(), size=(H, W), mode="nearest")
...
###the below is for resnet50unet with patches withyupp
# File: modeling.py
# --------------------------------------------------------
# Based on BEiT, timm, DINO and DeiT code bases
# https://github.com/microsoft/unilm/tree/master/beit
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
        ...
        if use_rpb:
            ...
            # --- THE FINAL FIX IS IN THIS BLOCK ---
            coords_h = torch.arange(window_size)
            coords_w = torch.arange(window_size)
...
# All @register_model functions remain the same
@register_model
def icolorit_tiny_4ch_patch8_224(pretrained=False, **kwargs):
    model = IColoriT(
        num_classes=128,
