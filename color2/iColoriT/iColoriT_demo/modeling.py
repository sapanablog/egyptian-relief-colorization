...
class IColoriT(nn.Module):
    ...
    """
###here for new env "icolorit_egypt" set up i have added here this extra pretrained_cfg=None
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=512, embed_dim=512, depth=12,
                 ...
                 use_rpb=False, avg_hint=False, head_mode='default', mask_cent=False,pretrained_cfg=None,pretrained_cfg_overlay=None,cache_dir=None):
        super().__init__()
        self.num_classes = num_classes
    ...
    def forward_features(self, x, mask):
        ...
        B, _, H, W = x.shape
        print('[DEBUG] Model got mask of shape:', mask.shape)
        if mask.dim() == 2:
            print('[DEBUG] ab channel min/max:', mask[:, 0].min().item(), mask[:, 0].max().item())
            print('[DEBUG] mask (channel2) unique:', mask[:, 2].unique())
