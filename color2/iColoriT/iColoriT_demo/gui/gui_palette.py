...
#####the above code is original one and now i have just changed for egyptologist pallete
...
# --- Egyptologist palette (RGB, scaled 0-1 for your code)
EGYPTOLOGIST_PALETTE = np.array([
    [30, 64, 120],     # Dunkelblau (Dark Blue)
    [104, 173, 213],   # Hellblau (Light Blue)
    [205, 60, 41],     # Rot (Red)
    [244, 205, 55],    # Gelb (Yellow)
    [0, 0, 0],         # Schwarz (Black)
]) / 255.0
...
class GUIPalette(QWidget):
    ...
    def __init__(self, grid_sz=(5, 1)):
        ...
        self.color_id = -1
        self.reset()
        self.set_colors(EGYPTOLOGIST_PALETTE)
    ...
    ##############################The above is original one and down is for checking pallete  as like egyptologist
