...
class GUIDraw(QWidget):
    ...
    def compute_result(self):
        ...
        print('[DEBUG] _im_lab shape:', _im_lab.shape, '_img_mask shape:', _img_mask.shape)
        print('[DEBUG] ab hints min/max:', _img_mask[:2].min(), _img_mask[:2].max())
        print('[DEBUG] mask channel unique:', np.unique(_img_mask[2]))
        #print('[DEBUG] mask channel unique:', np.unique(_img_mask[2].cpu()))

        # _im_lab is the full color image, _img_mask is the ab_hint+mask
        ab = self.model(_im_lab.unsqueeze(0), _img_mask.unsqueeze(0))
        print('[DEBUG] Model output ab shape:', ab.shape)
    ...
    def mousePressEvent(self, event):
        ...
        if pos is not None:
            if event.button() == Qt.LeftButton:
                print('[DEBUG] Left click:', event.pos())
                self.pos = pos
                self.ui_mode = 'point'
                self.change_color(pos)
                self.update_ui(move_point=False)
                print('[DEBUG] Current edits:', [vars(e) for e in self.uiControl.userEdits])
                self.compute_result()
