...
class IColoriTUI(QWidget):
    def __init__(self, color_model, img_file=None, load_size=224, win_size=256, device='cpu'):
        ...
        # === PATCH START: Egyptology Fixed Palette ===
        self.egypt_palette = {
            "Dunkelblau": "#006dcc",
            "Hellblau": "#8b9dc7",
            "Rot": "#a77c4e",
        ...
        self.palette_group = QGroupBox("Egyptology Colors")
        palette_layout = QVBoxLayout()

        for name, hex_color in self.egypt_palette.items():
            btn = QPushButton(name)
            btn.setStyleSheet(f"background-color: {hex_color}; color: white")
            #btn.clicked.connect(lambda checked, color=hex_color: self.set_fixed_color(color))
            btn.clicked.connect(self.make_color_callback(hex_color))

            palette_layout.addWidget(btn)

        self.palette_group.setLayout(palette_layout)
        colorLayout.addWidget(self.palette_group)
        # === PATCH END ===
    ...
    # === METHOD PATCH: set_fixed_color ===
    def set_fixed_color(self, hex_color):
        from PyQt5.QtGui import QColor
        color_style = f"background-color: {hex_color}"
        self.colorPush.setStyleSheet(color_style)

        color = QColor(hex_color)
        self.drawWidget.user_color = color
        self.drawWidget.color = color
        self.drawWidget.set_color((color.red(), color.green(), color.blue()))
    ...
    def make_color_callback(self, color):
        return lambda checked=False: self.set_fixed_color(color)
